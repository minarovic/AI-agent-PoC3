...
def test_studio_graph_compatibility():
    ...
    try:
        if memory_agent:
            ...
            print(
                f"✅ Graph visualization available: {len(graph_viz.nodes)} nodes, {len(graph_viz.edges)} edges"
            )
            success_checks.append(True)
        else:
            success_checks.append(False)
    ...
    try:
        if memory_agent and hasattr(memory_agent, "checkpointer"):
            print(
                f"✅ Checkpointer available: {type(memory_agent.checkpointer).__name__}"
            )
            success_checks.append(True)
        else:
        ...
        "langraph_studio_prompt_editing": {
            ...
            "method_2_langsmith_playground": {
                ...
                "configuration_fields": list(
                    get_agent_configuration_schema()["configurable"].keys()
                ),
            },
...
def main():
    ...
    test_results.append(
        ("LangSmith Playground", test_langsmith_playground_integration())
    )
    ...
    if passed >= 4:  # Allow for one test to be informational
        print(
            "✅ LangGraph Studio prompt editing capabilities are VERIFIED and WORKING"
        )
