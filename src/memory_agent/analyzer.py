...
def analyze_company_query(query: str) -> Tuple[str, str]:
    ...
    if not company_name:
        ...
        if ";" in query:
            ...
            if len(words) <= 3 and not any(word in query_lower for word in 
                ["what", "tell", "about", "analyze", "risk", "supplier", "chain"]):
    ...
    if any(keyword in query_lower for keyword in 
           ["risk", "rizik", "compliance", "sanction", "sankce"]):
        analysis_type = "risk_comparison"
    
    # Supplier analysis keywords  
    elif any(keyword in query_lower for keyword in 
             ["supplier", "dodavatel", "chain", "řetězec", "vztah", "relationship"]):
...
def format_analysis_data(company_data: Dict[str, Any], 
                        internal_data: Dict[str, Any], 
                        relationships_data: list) -> Dict[str, str]:
    """
    ...
    if company_data:
        ...
        external_data += f"- Země: {', '.join(company_data.get('countries', ['N/A']))}\n"
        
        if company_data.get('addresses'):
            external_data += f"- Adresa: {company_data['addresses'][0].get('full', 'N/A')}\n"
...
def analyze_company(query: str) -> str:
    ...
    try:
        ...
        formatted_data = format_analysis_data(company_data, internal_data, relationships_data)
