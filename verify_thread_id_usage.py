...
def print_report(
    file_path: str, total_ok: bool, issues: List[Tuple[int, str, bool]]
) -> None:
    """
...
def process_files(file_paths: List[str], auto_fix: bool = False) -> Dict[str, Any]:
    ...
    for file_path in file_paths:
        ...
        try:
            ...
            results["details"][file_path] = {"ok": total_ok, "issues": len(issues)}
    ...
    if results["files_with_issues"] > 0:
        logger.warning(
            "Některé soubory nemají správně implementované thread_id v konfiguraci."
        )
        logger.warning(
            "Pro správnou funkci checkpointů je potřeba přidat thread_id ke všem voláním graph.invoke()."
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Kontrola a oprava používání thread_id v Python souborech."
    )
    parser.add_argument("files", nargs="+", help="Soubory k zkontrolování")
    parser.add_argument(
        "--auto-fix",
        action="store_true",
        help="Automaticky opravit nalezené problémy (zatím nepodporováno)",
    )
