...
def test_current_graph_structure():
    ...
    print(
        f"Graph nodes: {list(memory_agent.nodes.keys()) if memory_agent.nodes else 'No nodes'}"
    )
...
def test_prompt_registry_functionality():
    ...
    print(
        f"Added prompt retrieved successfully: {retrieved_prompt == test_prompt_text}"
    )
...
def test_langgraph_studio_compatibility():
    ...
    try:
        ...
        print(
            f"Graph nodes in schema: {list(graph_schema.nodes.keys()) if graph_schema else 'None'}"
        )
...
def main():
    ...
    if passed == total:
        print(
            "✅ All tests passed - LangGraph Studio prompt editing should be supported"
        )
    else:
    ...
    print(
        "2. To support dynamic prompt editing, consider implementing ConfigurableFields"
    )
